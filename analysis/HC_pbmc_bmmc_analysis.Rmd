---
title: "HC - PBMC and BMMC data analysis"
author: "Kushal K Dey"
date: "1/14/2018"
...
pbmc_data <- pbmc$data
pbmc_labs <- pbmc$labs
idx1 <- which(bmmc_labs == "HC")
idx2 <- which(pbmc_labs == "HC")
...
```{r}
hc_pbmc_bmmc <- get(load("../output/hc_pbmc_bmmc_seurat.rda"))
```

```{r}
topic_clus_list <- get(load("../output/HC_pbmc_bmmc_countclust.rda"))
```
...
```{r}
tsne_vals <- hc_pbmc_bmmc@dr$tsne@cell.embeddings
```


```{r}
omega <- topic_clus_list[[2]]$omega
omega <- omega[match(hc_pbmc_bmmc@cell.names, colnames(aggr_data)),]
labs_clus <- factor(apply(omega, 1, function(x) return(which.max(x))))
data_ggplot <- data.frame(labels = labs_clus,
                          tSNE_1 = hc_pbmc_bmmc@dr$tsne@cell.embeddings[, 1],
                          tSNE_2 = hc_pbmc_bmmc@dr$tsne@cell.embeddings[, 2])
...
```{r}
new_labs_2 <- new_labs[match(hc_pbmc_bmmc@cell.names, colnames(aggr_data))]
