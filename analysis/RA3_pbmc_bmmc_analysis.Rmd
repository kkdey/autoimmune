...
pbmc_data <- pbmc$data
pbmc_labs <- pbmc$labs
idx1 <- which(bmmc_labs == "RA3")
idx2 <- which(pbmc_labs == "RA3")
...
```{r}
ra3_pbmc_bmmc <- get(load("../output/ra3_pbmc_bmmc_seurat.rda"))
```

```{r}
topic_clus_list <- get(load("../output/RA3_pbmc_bmmc_countclust.rda"))
```
...
omega <- omega[match(ra3_pbmc_bmmc@cell.names, colnames(aggr_data)),]
labs_clus <- factor(apply(omega, 1, function(x) return(which.max(x))))
data_ggplot <- data.frame(labels = labs_clus,
                          tSNE_1 = ra3_pbmc_bmmc@dr$tsne@cell.embeddings[, 1],
                          tSNE_2 = ra3_pbmc_bmmc@dr$tsne@cell.embeddings[, 2])
...
```{r}
new_labs_2 <- new_labs[match(ra3_pbmc_bmmc@cell.names, colnames(aggr_data))]
